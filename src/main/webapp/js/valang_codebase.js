            ...
            try {
              ...
              alert(ex);
                ...
                thisValidator.form.formElement.onsubmit = function() {
                    return (thisValidator.validateAndShowFeedback() && (!oldOnsubmit || oldOnsubmit()));
                }
            }
        }
    },
...
/* override method for showing errors */
var effectOptions = { duration : 0.25 };
var showErrors =
function(failedRules) {
  var i = 0;
  var html = [];
  html[i++] = "<div id='errorsListBox' class='errors' style='display:none'>";
  html[i++] = "<b>Please fix the following errors:</b>";
  html[i++] = "<ul>";
  for (var r = 0; r < failedRules.length; r++) {
    html[i++] = "<li>" + failedRules[r].getErrorMessage() + "</li>";
  }
  html[i++] = "</ul>";
  html[i++] = "</div>";
  ...
  if ($('errorsListBox').style.display == 'none') {
    Effect.Appear('errorsListBox', effectOptions);
  } else {
    new Effect.Highlight('errorsListBox');
  }
  scroll(0,0);
}
ValangValidator.prototype.showValidationFeedback = showErrors;
