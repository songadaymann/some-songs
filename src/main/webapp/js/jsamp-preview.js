...
function SMPlayer(oSoundPlayer) {
  ...
  this.oMain = $('player-template');
  if (!this.oMain) return;
  this.o = this.oMain.getElementsByTagName('div')[0];
  this.oLeft = this.o.select('div.left')[0];
  this.oBar = this.o.select('div.mid')[0];
  this.oSlider = this.o.select('a.slider')[0];
  this.oTitle = this.oBar.select('span.caption')[0];
  this.oSeek = this.oBar.select('div.seek')[0];
  this.oDivider = this.oBar.select('div.divider')[0];
  this.sFormat = (this.oTitle.innerHTML||'%artist - %title');
  this.sFormatSeek = (this.oSeek.innerHTML||'%{time1}/%{time2} (%{percent}%)');
  this.oProgress = this.oBar.select('div.progress')[0];
  this.oRight = this.o.select('div.right')[0];
  this.oTime = this.o.select('div.time')[0];
  this.oShuffle = this.o.select('a.shuffle')[0];
  this.oRepeat = this.o.select('a.loop')[0];
  this.oMute = this.o.select('a.mute')[0];
  this.oVolume = this.o.select('a.volume')[0];
  this.lastTime = 0;
  this.scale = 100;
  ...
  var useAltFont = false;//u.isMac; // specific letter-spacing CSS tweak (OSX has better-kerned/tighter spacing)

  // set default caption
  this.oTitle.innerHTML = this.oBar.select('div.default')[0].innerHTML;
  ...
  this.down = function(e) {
    ...
    self.didDrag = false;
    var e = e?e:event;
    self.offX = e.clientX-(self.oSlider.cumulativeOffset().left-self.oBar.cumulativeOffset().left);
    self.busy = true;
    $(self.oSlider).addClassName('active');
    self.refreshSeek();
    self.setSeekVisibility(1);
    document.observe('mousemove', self.move);
    document.observe('mouseup', self.up);
  ...
  this.barDown = function(e) {
    ...
    self.coords.offX = e.clientX-self.oMain.cumulativeOffset().left;
    self.coords.offY = e.clientY-self.oMain.cumulativeOffset().top;
    document.observe('mousemove', self.barMove);
    document.observe('mouseup', self.barUp);
    return false;
  }
  ...
  this.barUp = function(e) {
    document.stopObserving('mousemove', self.barMove);
    document.stopObserving('mouseup', self.barUp);
  }
  ...
  this.barClick = function(e) {
    ...
    var xNew = Math.min(e.clientX-self.oBar.cumulativeOffset().left,self.xMaxLoaded);
  ...
  this.volumeDown = function(e) {
    ...
    self.volumeX = self.oVolume.cumulativeOffset().left;
  ...
  this.setVolume = function(nVol) {
    ...
    self.oVolume.setOpacity(nVol/100);
  }
  ...
  this.up = function(e) {
    document.stopObserving('mousemove', self.move);
    document.stopObserving('mouseup', self.up);
    $(self.oSlider).removeClassName('active');
    self.busy = false;
...
// soundManager._writeDebug('setLoadingProgress(): '+nPercentage);
  ...
  this.setLoading = function(bLoading) {
    ...
    if (bLoading) {
      $(self.oProgress).addClassName('loading');
    } else {
      $(self.oProgress).removeClassName('loading');
    }
  ...
  this.loadingAnimate = function() {
    ...
    self.loadingX--;
    self.oProgress.setStyle('backgroundPosition', self.loadingX+'px '+self.loadingY+'px');
  ...
  this.loadingAnimateSlide = function() {
    ...
    self.oProgress.setOpacity(self.loadingTween[self.loadingAnimateFrame++]/100);
  ...
  this.setShuffle = function(bShuffle) {
    if (bShuffle) {
      $(self.oShuffle).addClassName('active');
    } else {
      $(self.oShuffle).removeClassName('active');
    }
  }

  this.setRepeat = function(bRepeat) {
    if (bRepeat) {
      $(self.oRepeat).addClassName('active');
    } else {
      $(self.oRepeat).removeClassName('active');
    }
  }

  this.setMute = function(bMute) {
    if (bMute) {
      $(self.oMute).addClassName('active');
    } else {
      $(self.oMute).removeClassName('active');
    }
  }
...
*/
  ...
  this.doScroll = function() {
    ...
    if (self.scrollOffset>self.coords.titleWidth) {
      ...
      self.scrollOffset = (Prototype.Browser.IE?5:1);
    }
  ...
  if (Prototype.Browser.WebKit) $(this.oMain).addClassName('noOpacity'); // stupid transparency tweak
//  if (useAltFont) this.oMain.addClassName('altFont');
...
//    self.reset();
  ...
  this.setOpacity = function(nOpacity) {
    ...
    // self.o.setOpacity(nOpacity);
  }

  this.createTweens = function() {
    // calculate tweens
    var base = 0;//(smUtils.isOldIE?16:0); // IE<7 needs vertical offset for playlist.
  ...
  this.setCoords = function(nHeight,nOpacity,nWidth) {
    ...
    if (!Prototype.Browser.IE) self.o.setOpacity(nOpacity/100);
  ...
  this.createPlaylist = function() {
    ...
    // assign copies
    self.playlistItemsUnsorted = self.playlistItems.clone();
    self.playlistItemsShuffled = self.playlistItems.clone();
  }
  ...
  this.loadFromHash = function() {
    ...
    if (index != -1) {
      ...
      self.play(index);
      window.observe('beforeunload',self.removeHash);
    } else {
...
function SPPLaylistItem(oLink,oPlaylist,nIndex) {
  ...
  var itemId = oLink.id.match(/\d+/);
  var songTitle = $('title'+itemId).innerHTML;
  var artistName = $('artist'+itemId).innerHTML;
  this.userTitle = '"'+songTitle+'" by '+artistName;
  ...
  this.setHighlight = function() {
    $(self.o).addClassName('highlight');
  }

  this.removeHighlight = function() {
    $(self.o).removeClassName('highlight');
  }
