  ...
  <div class="songListPaging">
    <c:if test="${replySearch.numPages > 1}">
      ...
      <c:forEach var="pageStartNum" items="${replySearch.pageStartNums}" varStatus="loopStatus">
        <c:url var="pageLink" value="/playlist">
          <c:param name="id" value="${playlist.id}"/>
          <c:param name="comment" value="${originalComment.id}"/>
          <c:param name="start" value="${pageStartNum + 1}"/>
        </c:url>
        <a href="<c:out value="${pageLink}"/>#comments"<c:if test="${replySearch.startResultNum eq pageStartNum}"> style="border: 1px solid black"</c:if>><c:out value="${loopStatus.index + 1}"/></a>
      </c:forEach>
    </c:if>
  </div>
...
<c:url var="replyThreadLink" value="/user/playlist_comment_reply">
  <c:param name="comment" value="${originalComment.id}"/>
</c:url>
