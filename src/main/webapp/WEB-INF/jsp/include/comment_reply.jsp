<%@ page import="com.ssj.model.song.SongCommentReply" %>
  ...
  <tr class="postTopRow">
    ...
    <td align="right">
      <c:if test="${reply.user.id eq user.id or isAdmin}">
        <c:url var="editReplyLink" value="/user/comment_reply">
      ...
      <%
        ...
        if (session.getAttribute("mqSongCommentReplyIds") != null) {
          SongCommentReply reply = (SongCommentReply) pageContext.getAttribute("reply");
          // TODO replace Set with Map so I can do all this with taglibs
          if (((Set) session.getAttribute("mqSongCommentReplyIds")).contains(reply.getId())) {
      ...
      <c:url var="quoteReplyLink" value="/user/comment_reply">
    ...
    <td colspan="2" class="postContent">
      ...
      <c:if test="${reply.moreCommentText ne null}">
        <c:url var="wholeReplyLink" value="/whole_post">
          <c:param name="type" value="songCommentReply"/>
          <c:param name="id" value="${reply.id}"/>
        </c:url>
