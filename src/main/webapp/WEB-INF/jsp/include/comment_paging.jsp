...
<div class="songListPaging">
  <c:set var="cBaseLink" value="/songs/${song.titleForUrl}-${song.id}"/>
  ...
  <span class="clickable" title="Jump to results" onclick="jumpPages(this, '<c:url value="${cBaseLink}"/>?id=<c:out value="${song.id}"/>', <c:out value="${commentSearch.totalResults}"/>, <c:out value="${commentSearch.resultsPerPage}"/>);">[jump...]</span>
  </c:if>
</div>
</c:if>