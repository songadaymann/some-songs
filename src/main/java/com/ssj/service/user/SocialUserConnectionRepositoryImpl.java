...
public class SocialUserConnectionRepositoryImpl implements ConnectionRepository {
  ...
  public void addConnection(Connection<?> connection) {
    ...
    // check if a local account has already been connected to this social account
    ...
    if (userIds.contains(userId)) {
      // user has already added this connection, may just be adding permissions, ignore for now
      return;
    }
    if (!userIds.isEmpty()) {
      // don't allow people to associate multiple local accounts with the same social account
      // if they do, we won't know what local account to log them in to when they try to log in
  ...
  public void removeConnections(String providerId) {
    List<SocialUser> socialUsers = socialUserDAO.findByUserIdAndProviderId(userId, providerId);
    // ok to delete one at a time as there should only be one account from any provider for a given user
    for (SocialUser socialUser : socialUsers) {
